...
#define DFC_NO_CLASSIFICATION

/*----------------------------------------------------*/
typedef struct mem_pair_t_ {
    void* host;
    void* dev;
} mem_pair_t;

typedef struct pk_gpu_t_ {
    uint32_t batch_size;
    mem_pair_t pk;
    mem_pair_t pk_size;
    mem_pair_t pk_base;
...
void Save_DFA(ACSM_STRUCT2 * acsm);
bool is_accept_state(ACSM_STRUCT2* acsm, int k);
