...
void acsmx2_print_qinfo(void)
{
    ...
    if( snort_conf->max_inq )
    {
        ...
        LogMessage("mpse: queue flushes  = " STDu64 "\n", (long long unsigned int)snort_conf->tot_inq_flush );
        LogMessage("mpse: queue inserts  = " STDu64 "\n", (long long unsigned int)snort_conf->tot_inq_inserts );
        LogMessage("mpse: queue uinserts = " STDu64 "\n", (long long unsigned int)snort_conf->tot_inq_uinserts );
    }
#endif
}
    ...
    { \
        ...
        { \
            ...
            { \
                index = T - mlist->n - Tx; \
                nfound++; \
                if (Match (mlist, mlist->rule_option_tree, mlist->n, T, mlist->neg_list) > 0) \
                { \
                    *current_state = state; \
                    return nfound; \
                } \
        ...
        case 1:
            {
				...
				{ 
					...
					{ 
						...
						{ 
							index = T - mlist->n - Tx; 
							int res = Match (mlist, mlist->rule_option_tree, mlist->n, T, mlist->neg_list);
							nfound += res; 
							if( res > 0 )
							{ 
								*current_state = state; 
